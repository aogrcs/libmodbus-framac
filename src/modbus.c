    ...
    case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb) {
            ...
            for (i = mapping_address, j = 6; i < mapping_address + nb; i++, j += 2) {
                /* 6 and 7 = first value */
                if(mb_mapping->tab_registers[i] != 0)
                {
    ...
    unsigned int start_input_registers, unsigned int nb_input_registers)
{
    ...
    mb_mapping = (modbus_mapping_t *)calloc(1, sizeof(modbus_mapping_t));
    if (mb_mapping == NULL) {
        return NULL;
    }
    ...
    if (nb_bits == 0) {
        ...
        mb_mapping->tab_bits =
            (uint8_t *) calloc(1, nb_bits * sizeof(uint8_t));
    ...
    if (nb_input_bits == 0) {
        ...
        mb_mapping->tab_input_bits =
            (uint8_t *) calloc(1, nb_input_bits * sizeof(uint8_t));
    ...
    if (nb_registers == 0) {
        ...
        mb_mapping->tab_registers =
            (uint16_t *) calloc(1, nb_registers * sizeof(uint16_t));
    ...
    if (nb_input_registers == 0) {
        ...
        mb_mapping->tab_input_registers =
            (uint16_t *) calloc(1, nb_input_registers * sizeof(uint16_t));
